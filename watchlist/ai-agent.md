...
|---|---|---|---|---|---|---|
| as-studio-agent | Android Studio Quail 4 Agent（AQI Fix with AI + LeakCanary Fix with Agent + 内置 23 skills + 并行 Agent） | Watch | Crash 高 / Leak 中高 / 迁移高 | 是（Crash/Leak/官方迁移） | Adopt | 每日 |
| crashlytics-mcp | Firebase Crashlytics MCP + `/crashlytics:connect` | Watch | Crash 高（Experimental） | 是（On-call 分诊） | Adopt | 每日 |
| android-cli-skills | Android CLI + android/skills 官方技能库（仍 24 个 SKILL.md） | Watch | 工具层完整 / 业务验证靠 Agent | 是（迁移/文档/设备） | Adopt | 每日 |
...
4. **Performance 分诊（新）**：录 Perfetto → `android-profiler` → `trace_processor` SQL → 根因报告。自动改代码+复测仍 Hold。
